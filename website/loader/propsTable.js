    ...
    matchedCode.forEach((code) => {
      if (/^<PropsTable[*<]*/.test(code)) {
        const {
          ...
          src,
          description,
        } = replaceSource.bind(this)(lastSource, code, options);
...
function replaceSource(source, code, options) {
  ...
  // console.log("propsTable ast code", JSON.stringify(result, null, 2));
  const { props } = result
    .map((re) => {
      if (re.name === "PropsTable") {
        ...
        if (!data && src) {
            ...
            Object.keys(options.alias).forEach((aliasName) => {
              if (RegExp(`^${aliasName.replace("$", "\\$")}.*`).test(src)) {
                src = resolve(
                  path.relative(
                    __dirname,
                    src.replace(aliasName, options.alias[aliasName])
                  )
                );
              }
            });
          ...
          try {
            ...
            const defaultCode = docSource.match(
              new RegExp(
                'defaultProps([0-9a-zA-Z \n";{}=<>,:;./*()|\u4e00-\u9fa5])*,\n};'
              )
            );
            if (defaultCode[0]) {
              defaultValueProps = eval(defaultCode[0]);
            }
...
/>`.replace(/"({require\('!!@tencent\/react-docgen-loader![^()]*'\)})"/g, "$1");
  ...
  return {
    ...
    src: props.src,
    description:
      props.showDescriptionOnSummary && props.data && props.data.description,
  };
}
